...
1. Start from a small seed tour.
2. For each unvisited city k, compute its distance to the current tour D[k][v]
3. Choose the city with minimum value. min D[k][v]
4. Insert it at the cheapest position.
...
while unvisited_count > 0:
    ...
    total_length = 0
    for r in range(len(completed_tour) - 1):
        total_length += instance.euclid_distance[completed_tour[r]][completed_tour[r+1]]

end_ni = time.perf_counter()

ni_time_ms = (end_ni - start_ni) * 1000
print(completed_tour) # [0, 21, 15, 7, 37, 30, 43, 6, 27, 36, 18, 26, 16, 42, 5, 29, 35, 17, 45, 32, 19, 11, 10, 46, 20, 31, 38, 47, 4, 23, 44, 34, 3, 25, 9, 41, 1, 28, 12, 24, 13, 22, 33, 40, 2, 14, 39, 8, 0]
print(total_length) # 37994.32137894776
